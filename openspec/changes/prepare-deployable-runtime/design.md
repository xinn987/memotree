...
- Provide staging-style deployment templates that wire API, Worker, Web, MySQL, and S3-compatible storage together.
...
- No new product behavior beyond deployment/runtime readiness.
...
- `memotree-worker`: Go Worker with FFmpeg/FFprobe installed.
- `memotree-web`: static Web build served by a small HTTP server such as Nginx or Caddy.

Rationale: API, Worker, and Web have different scaling, dependencies, and health characteristics. Worker needs FFmpeg; Web does not. API should expose `/healthz`; Worker should fail fast if dependencies are missing.

Alternatives considered:

- Single all-in-one image: simpler to run, but mixes FFmpeg, static assets, API process, and Worker lifecycle.
...
Rationale: image build coverage catches missing files, wrong Go versions, FFmpeg packaging issues, and static build assumptions without committing to registry or environment secrets.
...
4. Start services.
5. Run smoke tests for registration, family creation, upload, Worker processing, timeline, soft delete, and retry.
