...
- [ ] 2.1 Add staging-style deployment templates that wire API, Worker, Web, MySQL, and S3-compatible object storage together.
...
- [ ] 3.1 Document or add an explicit schema initialization command suitable for staging before long-running services start.
- [ ] 3.2 Document or add an explicit object storage bucket initialization command suitable for staging.
- [ ] 3.3 Clarify one-time initialization credentials versus long-running API/Worker credentials in deployment docs.
- [ ] 3.4 Document service logs, dependency logs, and rollback expectations for staging.

## 4. CI Readiness

- [ ] 4.1 Align GitHub Actions Go version with the repository-supported Go version.
- [ ] 4.2 Update CI and `tools/check.mjs` so OpenSpec validation targets current specs or all relevant active planning artifacts rather than archived changes.
- [ ] 4.3 Add CI image-build checks for API, Worker, and Web images without pushing or deploying them.
- [ ] 4.4 Keep Web type check/build and Go test coverage in CI.
