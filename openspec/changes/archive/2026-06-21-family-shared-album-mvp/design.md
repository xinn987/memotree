...
MemoTree 的第一阶段面向家庭宝宝照片和视频精选共享场景。当前目标不是构建 AI 相册、网盘、全量备份工具或 NAS 管理器，而是先解决家庭成员跨平台查看精选近况和上传精选照片视频的核心痛点。
...
- 功能范围需要收束，MVP 不做评论、点赞、收藏、AI、标签、地图、成长报告或复杂权限系统。
...
- 按时间线快速浏览家庭媒体。
- 支持查看媒体详情；原文件下载能力后置，但从 MVP 开始保存私有原文件以便未来支持。
- 以移动端 Web/PWA 体验优先，兼容 iOS、Android 和桌面浏览器。

**Non-Goals:**

- MVP 不做评论、点赞、收藏等家庭内轻互动，但后续版本可以考虑。
- 不做公开分享链接或家庭外社交传播。
...
- 核心后端职责是鉴权、家庭权限、上传签名、展示资源访问、媒体元数据和时间线查询，Go 很适合实现清晰可控的 API 服务。
...
### Decision: Keep Original Files Private And Reserve Future Download Access

原图和原视频保存在 Cloudflare R2 私有对象存储中，前端不持有长期公开 URL。MVP 只提供展示资源访问，不提供原文件下载入口或下载 API；后续如实现下载，必须经过 Go API 权限校验并使用短期授权。

Rationale:

- 防止复制出的媒体 URL 长期有效。
- 家庭成员被移除后，可以立即切断后续访问。
- 原文件下载后续可以通过短期签名 URL 或后端授权响应实现。
...
- 媒体浏览和上传的权限边界可以统一为 active family membership。
- 管理邀请和成员需要更高权限，因此只额外区分 admin。

Rules:

- `active member` 可以查看时间线、查看媒体详情和上传照片/视频。
...
- 原文件下载是后续兜底能力，MVP 不提供下载入口，但应保证原文件私有保存且不暴露长期公开 URL。
...
- 媒体处理不应运行在主 API 请求链路内，独立 Worker 可以避免缩略图、展示图和展示视频生成拖慢上传和浏览接口。
...
- MVP 不做评论、点赞和文字动态，因此没有必要引入“一个帖子包含多张媒体”的模型。
...
- 删除使用软删除状态；被删除媒体不出现在时间线或详情中。
- 对象存储中的原文件和展示资源可以等待后台清理或人工清理。

### Decision: Track Uploads As Returnable Upload Tasks

MVP 使用 `UploadBatch` 和 `UploadItem` 管理上传流程。上传任务可以返回查看结果，但不会作为时间线浏览对象；时间线只展示处理完成的 `MediaAsset`。

Rationale:

- 桌面端可能用于初始化上传一批精选内容，用户需要能返回查看任务结果。
- PWA 浏览器不能可靠承诺关闭页面后的原文件后台上传，但原文件上传完成后的媒体处理可以后台继续。
- 上传流程状态和媒体内容状态分离，可以避免首页出现大量处理中占位。
- 同一用户同一家庭限制一个 active 上传任务，可以降低前端队列、对象存储和 Worker 压力。

Domain model:

- `UploadBatch` 表示一次上传任务，记录家庭、创建者、整体状态、数量统计、创建时间和完成时间。
- `UploadItem` 表示任务中的单个文件，记录原始文件名、状态、错误信息和关联的 `MediaAsset`。
- `MediaAsset` 表示最终进入时间线的内容。

Rules:

- 同一 `User` 在同一 `Family` 内同一时间只能有一个 active `UploadBatch`。
- active 上传任务包括 created、uploading、processing 和 partially_failed。
- 用户再次点击上传时，如果已有 active 上传任务，应进入当前任务页。
- 上传者可以查看自己的上传任务；active admin 可以查看家庭内所有上传任务。
- 上传失败项需要重新上传原文件；处理失败项可以重新进入后端处理队列。
- 上传者或 active admin 可以停止上传任务；已完成的媒体保留，未完成项取消。
- 首页时间线只展示 `status=active` 且 `rendition_status=ready` 的 `MediaAsset`。
...
- 媒体处理会引入异步状态 -> 上传任务页显示处理中和失败状态，首页时间线只显示已处理完成的媒体。
