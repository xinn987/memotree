...
`, input.Width, input.Height, input.MediaAssetID, dbOriginalTypeImage); err != nil {
	...
	for _, rendition := range input.Renditions {
		if _, err := tx.ExecContext(ctx, `
INSERT INTO media_renditions (media_asset_id, rendition_type, object_key, content_type, byte_size, width, height, status, created_at, updated_at)
VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
...
`, input.MediaAssetID, rendition.RenditionType, rendition.ObjectKey, rendition.ContentType, rendition.ByteSize, rendition.Width, rendition.Height, dbRenditionStatusReady, now, now); err != nil {
			return err
		}
	}
	if _, err := tx.ExecContext(ctx, `
