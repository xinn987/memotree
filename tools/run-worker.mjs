...
import { ensureManagedFFmpegEnv, ensureTool, localMySQLDSN, localStorageEnv, printHelp, projectEnv, run, serverDir } from "./shared.mjs";
...
const env = ensureManagedFFmpegEnv(projectEnv({
  ...withLocalStorageDefaults(),
  MYSQL_DSN: process.env.MYSQL_DSN || localMySQLDSN,
}));

console.log(`MYSQL_DSN: ${env.MYSQL_DSN}`);
console.log(`FFMPEG_PATH: ${env.FFMPEG_PATH}`);
console.log(`FFPROBE_PATH: ${env.FFPROBE_PATH}`);
